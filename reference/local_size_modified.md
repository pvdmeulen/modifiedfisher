...
- .rejection_matrix:
  ...
  [`size_modified()`](https://pvdmeulen.github.io/modifiedfisher/reference/size_modified.md)
  does across its zoom grid) to avoid rebuilding it on every call. If
  supplied, it must correspond to the same `.df` and `.gamma0` passed
  alongside it; this is not checked.
...
for the local size under alternative tests at the same nuisance
parameter value;
[`size_modified()`](https://pvdmeulen.github.io/modifiedfisher/reference/size_modified.md)
for the modified local size maximised over the nuisance parameter;
[`power_modified()`](https://pvdmeulen.github.io/modifiedfisher/reference/power_modified.md)
for the power of the modified Fisher exact test;

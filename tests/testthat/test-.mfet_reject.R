# .mfet_reject() indexes its test frame by t + 1 (row t+1 holds the critical
...
test_that(".mfet_reject covers all five branches", {

  df <- make_stub_frame(t_target = 5, c1 = 2, c2 = 4, gamma1 = 0.3, gamma2 = 0.7)

  # strictly interior -> accept
  expect_equal(modifiedfisher:::.mfet_reject(c(3, 5), df, .gamma0 = 0.5), 0)
  # strictly exterior -> reject
  expect_equal(modifiedfisher:::.mfet_reject(c(0, 5), df, .gamma0 = 0.5), 1)
  expect_equal(modifiedfisher:::.mfet_reject(c(6, 5), df, .gamma0 = 0.5), 1)
  # at c1, gamma1 = 0.3 < gamma0 = 0.5 -> accept
  expect_equal(modifiedfisher:::.mfet_reject(c(2, 5), df, .gamma0 = 0.5), 0)
  # at c2, gamma2 = 0.7 > gamma0 = 0.5 -> reject
  expect_equal(modifiedfisher:::.mfet_reject(c(4, 5), df, .gamma0 = 0.5), 1)

})

test_that(".mfet_reject handles c1 == c2", {

  df <- make_stub_frame(t_target = 5, c1 = 3, c2 = 3, gamma1 = 0.2, gamma2 = 0.4)

  # gamma1 + gamma2 = 0.6 > gamma0 = 0.5 -> reject
  expect_equal(modifiedfisher:::.mfet_reject(c(3, 5), df, .gamma0 = 0.5), 1)
  # gamma1 + gamma2 = 0.6 < gamma0 = 0.7 -> accept
  expect_equal(modifiedfisher:::.mfet_reject(c(3, 5), df, .gamma0 = 0.7), 0)

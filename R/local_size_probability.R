...
#' \eqn{(i, j)}, rejection is determined by \code{pvalue_probability()}: the
...
#'
#' @return A single numeric value: the local size of the SAS Proc FREQ exact
#'   test at the given nuisance parameter, in \eqn{[0, 1]}.
#' @examples
...
#' @keywords internal
#' @family size
local_size_probability <- function(nuisance, .odds_ratio, .m, .n, .alpha) {
  ...
  for (i in 0:.m) {
    for (j in 0:.n) {
      ...
      t      <- i + j
      p_val  <- pvalue_probability(i, t, .m, .n, .odds_ratio)
